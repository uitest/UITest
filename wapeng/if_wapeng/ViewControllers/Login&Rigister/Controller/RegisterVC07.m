...
-(void)httpRequestWithPage:(int)newPage{
    ...
    __weak RegisterVC07 * weakSelf = self;
    ...
    [http thirdRequestWithUrl:dUrl_PUB_1_3_1 succeed:^(NSObject *obj, BOOL isFinished) {
        ...
        if ([sValue isEqualToString:@"<null>"]) {
            ...
            UIAlertView *alert=[[UIAlertView alloc]initWithTitle:@"提示" message:msg delegate:weakSelf cancelButtonTitle:@"重搜" otherButtonTitles:@"确定", nil];
...
//        }
        ...
        for (NSDictionary * dict in array) {
            ...
            item.kindergarten_Id = [dict objectForKey:@"id"];
            
            item.kindergartenLeader = @"";
            if (isNotNull([dict objectForKey:@"userInfo"])) {
                if (isNotNull([[dict objectForKey:@"userInfo"] objectForKey:@"id"])) {
                   item.kindergartenLeader = [[dict objectForKey:@"userInfo"] objectForKey:@"id"];
                }
            }
           
            [weakSelf.dateSource addObject:item];
        }
    ...
    } andKeyValuePairs:@"kindergartenQuery.longitude",self.longtitude,@"kindergartenQuery.latitude",self.latitude,@"kindergartenQuery.name",self.kind_Name,@"kindergartenQuery.pageNum",page,@"kindergartenQuery.zoneAreaID",areaID, nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"%@", rm.kindergartenLeader);
    
    if ([rm.kindergartenLeader isEqualToString:@""]) {
        NSLog(@"园长是不存在，没有班级信息");
        
        [[NSNotificationCenter defaultCenter] postNotificationName:dNoti_changeBabyKinder object:rm];
        [self.navigationController popViewControllerAnimated:YES];
        
    }
     r8._id=rm.kindergarten_Id;
    r8.kindergartenLeader = rm.kindergartenLeader;
