...
-(void)checkMobileData:(NSString *)str
{
    [http thirdRequestWithUrl:dUrl_ACC_1_1_1 succeed:^(NSObject *obj, BOOL isFinished) {
        ...
        if (![value isEqualToString:@"0"]) {
            // [SVProgressHUD showSuccessWithStatus:@"该手机号已经注册"];
            [timer invalidate];
        ...
        [SVProgressHUD showSuccessWithStatus:@"手机号验证网络请求失败!!"];
...
-(void)submitBtnOnClick
{
    ...
    //        [SVProgressHUD showSuccessWithStatus:@"请同意协议"];
    //        return;
    //    }
    ...
    if (_dataManager.regInfoID.length==0||self.msgCheckText.text.length<4) {
        [SVProgressHUD showSuccessWithStatus:@"请输入正确的验证码"];
    ...
    //}//else{
    //  [SVProgressHUD showSuccessWithStatus:@"验证码不对"];
    //}
}
...
- (IBAction)submitBtnClick:(id)sender {
    ...
    if (self.phoneNumText.text.length == 0) {
        ...
        [SVProgressHUD showSuccessWithStatus:@"请输入手机号"];
        return;
    }
