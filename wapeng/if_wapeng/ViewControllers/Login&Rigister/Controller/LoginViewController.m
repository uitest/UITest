...
-(void)checkMobileWithPhoneNum:(NSString *)phoneNum
{
    ...
    if (!isMatch) {
        [SVProgressHUD showSuccessWithStatus:@"手机号码格式输入不对"];
        return;
    }else{
        [afn thirdRequestWithUrl:dUrl_ACC_1_1_1 succeed:^(NSObject *obj, BOOL isFinished){
            ...
            if (![value isEqualToString:@"0"]) {
                ...
                checkFlag=@"0";
                [SVProgressHUD showSuccessWithStatus:@"该手机号尚未注册"];
                return;
            }
            
        } failed:^(NSObject *obj, BOOL isFinished) {
            NSLog(@"test register failed------");
            [SVProgressHUD showSuccessWithStatus:@"核对手机号是否注册网络请求失败"];
            return;
...
-(void)btnLoginClick:(UIButton *)sender
{
    ...
    if ([checkFlag isEqualToString:@"1"]&&self.passwordText.text.length!=0) {
        ...
        [SVProgressHUD showSuccessWithStatus:@"请先输入用户名或密码"];
        return;
    }
}
...
//        }
        ...
        [SVProgressHUD showSuccessWithStatus:@"手机号没有注册"];
