...
@interface MyDatumVC ()<UITextFieldDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,UIAlertViewDelegate, UITextFieldDelegate, PassValueDelegate, UIActionSheetDelegate>
{
...
//        [self changeIconView];
        ...
        [weakSelf.as showInView:weakSelf.view];
...
//    numAccount.text = @"123456";
    ...
    UILabel *address = [self addLabel:addStr y:CGRectGetMaxY(reminder.frame) + 5 hide:NO];
    
    address.tag = dTag_address;
...
//    addressContent.borderStyle = UITextBorderStyleLine;
    ...
    UIActionSheet * as = [[UIActionSheet alloc]initWithTitle:nil delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"拍照选择", @"从相册选择", nil];
    as.delegate = self;
    self.as = as;
}
...
-(void)backClick:(UIButton *)button
{
    ...
    if (self.type == 2) {
        ...
        if ([self.delegate respondsToSelector:@selector(changeHeaderPicDelegate:)]) {
            
            [self.delegate changeHeaderPicDelegate:self.image];
        }
