...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == temp.count + 1) {
        ...
        [SVProgressHUD showSuccessWithStatus:@"点击加载更多"];
...
-(void)sendMsgClick
{
    [SVProgressHUD showSuccessWithStatus:@"发私信"];
     Item_MyWindow * item = [self.dataArray lastObject];
    MyChatVC * myChatVC = [[MyChatVC alloc]init];
    
    myChatVC.title = item.petName;
    
    myChatVC.peerID = self.authorID;
