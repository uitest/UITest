...
-(void)getMsgClick
{
    ...
    [http thirdRequestWithUrl:dUrl_ACC_1_1_1 succeed:^(NSObject *obj, BOOL isFinished) {
        ...
        if ([value isEqualToString:@"0"]){
            [SVProgressHUD showSuccessWithStatus:@"手机号码没有被注册"];
            isRegister = NO;
        }else{
            //手机号码被注册过
            isRegister = YES;
...
-(void)nextStepClick
{
    ...
    if (tf1.text.length == 0 || tf1.text.length != 11) {
        ...
        [SVProgressHUD showSuccessWithStatus:@"手机号输入有误"];
        
                return;
    }
    if (tf2.text.length != 0 && [tf2.text isEqualToString:self.code]) {
        
                [SVProgressHUD showSuccessWithStatus:@"附加码输入正确"];
        return;
        
    }
    if (tf3.text.length == 0) {
        
        [SVProgressHUD showSuccessWithStatus:@"请输入短信验证码"];
