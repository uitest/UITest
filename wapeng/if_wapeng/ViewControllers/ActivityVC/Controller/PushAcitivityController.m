...
@interface PushAcitivityController ()<DoImagePickerControllerDelegate,UITextViewDelegate,faceViewDelegate,PassValueDelegate>
{
    ...
    CGRect                  _oldFrame;
    int                     _isAnonmity;//是否匿名
    NSString                *_placeName;//定位地点名称
}
...
@property (nonatomic, assign) BOOL anonymousButtonState;
...
//    _validity = ;
    ...
    //添加位置button
//    self.locationButton = [[UIButton alloc]initWithFrame:CGRectMake(CGRectGetMaxX(validity.frame) + 35, validity.frame.origin.y - 1, 85, 18)];
//    [self.locationButton setTitle:@"添加位置" forState:UIControlStateNormal];
//    self.locationButton.titleLabel.textAlignment = NSTextAlignmentLeft;
//    self.locationButton.titleEdgeInsets = UIEdgeInsetsMake(0, 25, 0, 0);
//    self.locationButton.titleLabel.font = [UIFont systemFontOfSize:13];
//    [self.locationButton setBackgroundImage:[UIImage imageNamed:@"addLocation"] forState:UIControlStateNormal];
//    [self.locationButton addTarget:self action:@selector(addLocation) forControlEvents:UIControlEventTouchUpInside];
...
//    [self.view addSubview:self.locationButton];
    ...
    UIImageView *img = [[UIImageView alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(_imageVessel.frame) + 3, kMainScreenWidth, 1)];
    ...
    //匿名发布button
    if (self.type == 1) {
        self.anonymousButton = [[UIButton alloc]initWithFrame:CGRectMake(kMainScreenWidth * 0.7, 0, 70, 25)];
        CGPoint centerPoint =  self.anonymousButton.center;
        centerPoint.y = _viewBg.frame.size.height * 0.5;
        self.anonymousButton.center = centerPoint;
        
        self.anonymousButtonState = YES;
        _isAnonmity = 2;
        [self.anonymousButton setBackgroundImage:[UIImage imageNamed:@"niming-notselected"] forState:UIControlStateNormal];
        [self.anonymousButton addTarget:self action:@selector(isAnonymousButton:) forControlEvents:UIControlEventTouchUpInside];
        
    }
    
    [_viewBg addSubview:self.anonymousButton];
//    
...
#pragma mark - 是否匿名
-(void)isAnonymousButton:(id)sender
{
    NSLog(@"------------ 是否匿名");
    self.anonymousButtonState = !self.anonymousButtonState;
    if (self.anonymousButtonState == YES) {
        [self.anonymousButton setBackgroundImage:[UIImage imageNamed:@"niming-notselected"] forState:UIControlStateNormal];
        _isAnonmity = 2;
    }else{
        [self.anonymousButton setBackgroundImage:[UIImage imageNamed:@"niming-selected"] forState:UIControlStateNormal];
        _isAnonmity = 1;
    }
...
-(void)sendLon:(NSString *)lon WithLat:(NSString *)lat WithPlaceName:(NSString *)name
{
    NSLog(@"%@", name);
    if (name) {
//        CGRect frame = CGRectMake(self.locationButton.frame.origin.x, self.locationButton.frame.origin.y, 150, 18);
...
-(void)publicAction{
    ...
    if (self.fastTextView.text.length == 0 || [self.fastTextView.text isEqual:[NSNull null]]) {
...
//    selectVC.isAnonmity = _isAnonmity;//是否匿名
//    selectVC.placeName = _placeName;
//    
//    
...
-(void)publicTop
{
    ...
    for (NSString * str in _imageIDArr) {
    ...
    NSString * newlimitTime = [TimeTool getmyLimitTime:@"3650"];
...
//    NSLog(@"searchText:%@", searchText);
    ...
    NSString * flag = [NSString stringWithFormat:@"%d", _isAnonmity];
    
    NSString * nullStr = @"";
    NSMutableDictionary * postDict = [[NSMutableDictionary alloc]initWithObjectsAndKeys:ddid, @"D_ID", _titleText.text, @"topic.title", [PublicStringTool unifyStringWithAndiordString:_contentTextStr], @"topic.content",nullStr, @"topic.searchWord", lon, @"topic.longitude", lat, @"topic.latitude", newlimitTime, @"topic.limitTime", flag,@"topic.anonymousFlag", nullStr,@"topic.topTopicLabelList[0].id",nil];
    ...
    [http fiveReuqestUrl:TOP_1_2_6 postDict:postDict succeed:^(NSObject *obj, BOOL isFinished) {
        ...
        //返回列表
        for (UIViewController * hotTopicVC in weakSelf.navigationController.viewControllers) {
            if ([hotTopicVC isKindOfClass:[HotTopicViewController class]]) {
                [weakSelf.navigationController popToViewController:hotTopicVC animated:YES];
            }
        }
    } failed:^(NSObject *obj, BOOL isFinished) {
...
//    //    [postDict addEntriesFromDictionary:commonDict];
//    
//    NSMutableDictionary * postDict = [[NSMutableDictionary alloc]initWithObjectsAndKeys:ddid, @"D_ID", self.mTitle, @"activity.title", [PublicStringTool unifyStringWithAndiordString:self.content], @"activity.content", searchText, @"activity.searchWord", lon, @"activity.longitude", lat, @"activity.latitude", newlimitTime, @"activity.limitTime",nil];
//    
