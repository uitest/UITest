...
-(void)connection
{
    ...
    [http fiveReuqestUrl:dUrl_OSM_1_1_3 postDict:postDict succeed:^(NSObject *obj, BOOL isFinished) {
        ...
        NSUserDefaults * d = [NSUserDefaults standardUserDefaults];
        NSDictionary * dict = [d objectForKey:UD_loginDict];
        NSString * userTypeID = [dict objectForKey:@"userTypeID"];
        item.userType = userTypeID.intValue;
        NSLog(@"%d", item.userType);
        ...
        if (item.userType == PARENT_USER || item.userType == TEACHER_USER) {
            ...
            item.personnelSignature = kNullData;
            
            if (isNotNull([root objectForKey:@"value"])) {
                if (isNotNull([[root objectForKey:@"value"] objectForKey:@"userInfoExtension"])) {
                    if (isNotNull([[[root objectForKey:@"value"] objectForKey:@"userInfoExtension"] objectForKey:@"personnelSignature"])) {
