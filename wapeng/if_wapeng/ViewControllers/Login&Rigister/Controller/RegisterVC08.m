...
-(void)httpRequestWithPageIndex:(int)page
{
    ...
    NSDictionary * postDict = [NSDictionary dictionaryWithObjectsAndKeys:self.kindergartenLeader, @"organizationBranchQuery.organizationID",pageStr,@"ctivityQuery.pageNum", @"2", @"organizationBranchQuery.dormant",nil];
    ...
    [http fiveReuqestUrl:dUrl_OSM_1_2_1 postDict:postDict succeed:^(NSObject *obj, BOOL isFinished) {
        ...
        for (NSDictionary * dict in list) {
            ...
            item.kindID = weakSelf._id;
            
            NSLog(@"%@", item.kindID);
            
            item.kindName = weakSelf.name;
            item.name = [dict objectForKey:@"name"];
            item._id = [dict objectForKey:@"id"];
            item.kindergatenLeader = weakSelf.kindergartenLeader;
...
//
//    } failed:^(NSObject *obj, BOOL isFinished) {
//        [SVProgressHUD showSuccessWithStatus:@"传参有误，请重试"];
//        [_tableView headerEndRefreshing];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.type == 1) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:dNoti_changBabyClass object:item];
